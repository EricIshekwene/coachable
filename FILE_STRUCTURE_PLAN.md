...
> Keep PascalCase (`AdminUsersPage.jsx`) or switch to lowercase (`users.jsx`)?
...
> A) `pages/auth/` + `pages/public/` subfolders
> B) Leave auth and public pages flat in `pages/`
...
> B) Move to `src/components/slate/` (simpler, stays under components/)
