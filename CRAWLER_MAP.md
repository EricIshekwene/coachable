...
| User says... | Primary file(s) | Notes |
|---|---|---|
| "tutorial", "product tour", "spotlight overlay", "onboarding walkthrough" | [src/components/tutorial/TutorialOverlay.jsx](src/components/tutorial/TutorialOverlay.jsx) | Full-screen animated spotlight overlay; portals to `document.body`; blocks all interaction outside the spotlighted target while the tour runs (four-panel click-trap via `blockerPanels`; only the spotlight hole, the card, and the exit button are clickable); top-left "Exit tutorial" button; per-step "Next: do it for me" auto-perform button; ghost Back button (left of card, hidden on step 1 and finish steps) navigates to the previous step's route and dispatches BACK; back-step support; in-card animated progress bar under the "Step X of N" label (`tutorialProgressPercent`); animated step transitions — card glides between positions, content cross-fades, delayed "Loading…" (`contentFadePhase`, `cardBodyMode`), reduced-motion respected; see [TUTORIAL_OVERLAY.md](src/components/tutorial/TUTORIAL_OVERLAY.md) |
| "tutorial state", "tour context", "useTutorial" | [src/context/TutorialContext.jsx](src/context/TutorialContext.jsx) | `TutorialProvider`/`useTutorial()`; outcome-verified advancement (collects tutorialBus events per step, evaluates `isComplete`) + `performStepAction()`; mounted in App.jsx above the router; in preview mode, exit/finish returns to `/admin` |
| "tutorial events", "tutorial actions", "tour bus" | [src/context/tutorialBus.js](src/context/tutorialBus.js) | Pure event/action bus: pages + Slate emit outcome events ("player-added", "keyframe-added", "invite-sent"...) and register auto-perform actions ("place-player", "draw-route"...) |
| "tutorial steps", "tour script" | [src/context/tutorialSteps.js](src/context/tutorialSteps.js) | Pure step data + reducer; `getTutorialSteps(user)` adapts to the sport (`getSportCapabilities`: mode-selection step for Football, prefab step for all non-Blank sports, keyframe flow, back-button finish) and skips the invite flow for solo/personal-team accounts |
| "tutorial preview mock", "fake session", "mock API" | [src/utils/tutorialPreview.js](src/utils/tutorialPreview.js) | In-memory mock backend for the admin preview; `apiFetch` routes every request here when the sessionStorage flag is set — nothing touches the server/DB; fake team uses the sport chosen in the admin picker; serves sample sport prefab presets for every real sport |
...
| "preview onboarding tutorial" (admin) | [src/pages/Admin.jsx](src/pages/Admin.jsx) — "Preview Onboarding Tutorial" sport-picker button (`TutorialPreviewMenu`) on the dashboard header | Pick one of the 9 sports → arms the mock preview flag + sport, then opens `/app/plays?startTutorial=1`; no server endpoint, no demo account; see [TUTORIAL_OVERLAY.md](src/components/tutorial/TUTORIAL_OVERLAY.md) |
...
| "has my team moved", "migration status context" | [src/context/MigrationStatusContext.jsx](src/context/MigrationStatusContext.jsx) (fetches `GET /migration/me` per user, then refreshes in the background every 60s like `NotificationsContext`; fails open, and a failed refresh keeps the last known good answer) + [src/utils/migrationDestination.js](src/utils/migrationDestination.js) (`V2_APP_URL`, `shouldShowMovedInterstitial`, `shouldShowMovedBanner`) |
...
| "print plays", "print preview", "2/4/6-up printing", "print sheet styles" | [src/components/printing/PrintPlaysOverlay.jsx](src/components/printing/PrintPlaysOverlay.jsx) — full-screen print preview opened from the bulk bar on `/app/plays`, with selectable sheet styles (Minimal / Sideline / Playcall / Gameday); pure pagination/layout/style helpers (`PRINT_LAYOUTS`, `PRINT_STYLES`) in [printLayout.js](src/components/printing/printLayout.js); gated by the `printing` suite feature (coach-only, fail closed) — see [docs/printing.md](docs/printing.md) (plan: [PRINTING_PLAN.md](PRINTING_PLAN.md)) |
...
| User says... | Primary file(s) | Notes |
|---|---|---|
| "account has moved", "go to the new Coachable", "cutover interstitial", "moved-team banner" | [src/pages/AccountMovedPage.jsx](src/pages/AccountMovedPage.jsx) | Full-screen "your team has moved" page (MaintenancePage styling, plus a `Log out` button and an error toast if a team switch fails). Gated by `RequireNotMoved` in [src/App.jsx](src/App.jsx); status from [src/context/MigrationStatusContext.jsx](src/context/MigrationStatusContext.jsx) via `GET /migration/me`; destination constant `V2_APP_URL` + pure show/hide helpers in [src/utils/migrationDestination.js](src/utils/migrationDestination.js); partially-migrated coaches get [src/components/MovedTeamBanner.jsx](src/components/MovedTeamBanner.jsx) instead. Fails open. See [MIGRATION_INTERSTITIAL.md](src/pages/MIGRATION_INTERSTITIAL.md) |
...
| "why can't I save", "409 TEAM_MIGRATED_TO_V2", "the write lock", "read-only after migration" | [server/middleware/migrationWriteLock.js](server/middleware/migrationWriteLock.js) | Global middleware mounted in `index.js` before every route. Refuses POST/PUT/PATCH/DELETE with **409 + `code: "TEAM_MIGRATED_TO_V2"`** when the target team is `v2_live` or `migrating`. Reads (GET/HEAD/OPTIONS) are never blocked; auth/account/admin/telemetry paths are on an explicit exemption allow-list; fails OPEN on any error or while the cache has never loaded. See [MIGRATION_WRITE_LOCK.md](server/middleware/MIGRATION_WRITE_LOCK.md) |
...
### Entrypoint
- [index.js](server/index.js) — Express app, CORS, route mounting, static `dist/` serving, auto-migrate, cleanup intervals, migration-status polling (`startMigrationStatusPolling()`), cutover write lock (`migrationWriteLock`, mounted after `bodyBoundsCheck` and before all route mounts)
...
| `/migration` (`GET /migration/me` — the authed user's per-team V1→V2 status, read from the cache module only) | [routes/migration.js](server/routes/migration.js) |
...
- [middleware/migrationWriteLock.js](server/middleware/migrationWriteLock.js) — V1→V2 cutover write lock: global middleware that answers 409 `TEAM_MIGRATED_TO_V2` to mutating requests for a `v2_live`/`migrating` team. Reads always allowed, explicit `EXEMPT_PATH_PREFIXES` allow-list, fails open on every error path. See [MIGRATION_WRITE_LOCK.md](server/middleware/MIGRATION_WRITE_LOCK.md)
...
- [routes/migration.js](server/routes/migration.js) — `GET /migration/me`: the authed user's teams + V1→V2 status from the in-memory cache. Read-only, never calls V2, always a GET so the cutover write-lock cannot block it
...
- V1→V2 cutover: `migrationStatusCache.test.js` (migration-status cache — contract-version refusal, payload validation, fail-to-last-known-good on every poll failure, 401/403 wrong-secret path, missing-`V2_MIGRATION_CRON_SECRET` no-crash path, secret never logged, staleness, user→team resolution via V1's own memberships — see [MIGRATION_STATUS_CACHE.md](server/lib/MIGRATION_STATUS_CACHE.md)), `migrationWriteLock.test.js` (cutover write lock — reads never blocked, 409 + stable `TEAM_MIGRATED_TO_V2` code on locked teams, `migrating` treated as locked, exemption allow-list, path/body team resolution, membership fallback for unattributed writes, fail-open on a never-loaded cache and on a DB error — see [MIGRATION_WRITE_LOCK.md](server/middleware/MIGRATION_WRITE_LOCK.md)), `migrationInterstitial.test.js` (moved-account interstitial — beta-host destination constant, all-moved vs mixed active-team behavior, interstitial/banner mutual exclusion, fail-open with no data, `GET /migration/me` shape + never calling V2 — see [MIGRATION_INTERSTITIAL.md](src/pages/MIGRATION_INTERSTITIAL.md))
...
- [docs/](docs/) — `PLATFORM_PLAYS.md`, `backend-flows-and-required-tables.md`, `team-suite.md`, `printing.md`
...
- [src/pages/MIGRATION_INTERSTITIAL.md](src/pages/MIGRATION_INTERSTITIAL.md) — the moved-account interstitial: `GET /migration/me`, the client context, the all-moved vs partially-moved behavior, the `V2_APP_URL` destination rule, and the fail-open policy
...
- [server/middleware/MIGRATION_WRITE_LOCK.md](server/middleware/MIGRATION_WRITE_LOCK.md) — V1→V2 cutover write lock: the 409 contract and `TEAM_MIGRATED_TO_V2` code, why `migrating` blocks writes, the exemption allow-list and its rationale, how a request is resolved to a team, and the fail-open guarantees
