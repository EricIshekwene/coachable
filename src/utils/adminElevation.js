...
export function isAdminElevated() {
  const val = sessionStorage.getItem(ELEVATED_KEY);
  if (!val) return false;
...
export function getAdminElevatedUntil() {
  return Number(sessionStorage.getItem(ELEVATED_KEY) || 0);
}
...
export function setAdminElevated(elevatedUntil) {
  sessionStorage.setItem(ELEVATED_KEY, String(elevatedUntil));
}
