...
function SlateRecord({
  ...
  // Which player/ball is focused in the recording timeline (for segment highlighting)
  const [focusedRecordingId, setFocusedRecordingId] = useState(null);
  ...
  useEffect(() => {
    setAnimationDataWithMeta((base) => {
      ...
      Object.entries(base.tracks || {}).forEach(([itemId, track]) => {
        // Always drop keys that are not valid strings (e.g. "[object Object]" from a bug
        // where a non-string value was accidentally used as a player ID).
  ...
  /**
   * Reflects all player/ball positions and animation keyframes across the given axis.
   * @param {"x"|"y"} axis - Axis to reflect across.
   */
  ...
  return (
    <>
      ...
      <div ref={containerRef} data-slate-root className="flex-1 flex relative">
        ...
        {showViewOverlay ? (
          ...
          <RecordingTimelinePill
            ...
            playersById={entities.playersById}
            ballsById={entities.ballsById}
            focusedId={focusedRecordingId ?? recording.recordingPlayerId}
            currentTimeMs={timelineDisplayTimeMs}
            onSeek={seekTimeline}
            getAuthoritativeTimeMs={getAuthoritativeTimeMs}
            getPreviewTimeMs={recording.getPreviewTimeMs}
            onDragStateChange={handleTimelineDragStateChange}
      ...
      ><RightPanel
        ...
        onRecordingModeToggle={recording.setRecordingModeEnabled}
        ...
        recordingFocusedId={focusedRecordingId}
        onRecordingFocusId={setFocusedRecordingId}
      /></div>}
      {!viewOnly && (
        <>
          <input
      ...
      {!viewOnly && showAdvancedSettings && (
        <AdvancedSettings
          ...
          recordingDebug={{
            ...
            previewTimeMs: recording.previewTimeMs,
            recordingDurationMs: recording.recordingDurationMs,
            stabilization: recording.stabilization,
            recordedCount: recording.recordedCount,
            totalCount: recording.totalCount,
            playerStates: recording.playerStates,
            playersById: entities.playersById,
            getDebugSnapshot: recording.getDebugSnapshot,
            animationData,
          }}
        />
      )}
