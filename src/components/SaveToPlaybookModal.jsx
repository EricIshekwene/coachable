...
export default function SaveToPlaybookModal({
  ...
  return (
    <div className={POPUP_MODAL_OVERLAY_CLASS} onClick={onClose}>
      <div
        className={`relative w-80 sm:w-96 p-5 sm:p-6 flex flex-col gap-4 max-h-[90vh] overflow-y-auto hide-scroll ${POPUP_SURFACE_CLASS}`}
      ...
      >
        ...
        {/* Preview thumbnail */}
        {thumbnailDataUrl && (
          <div className="w-full rounded-lg overflow-hidden border border-BrandGray2/60 bg-BrandBlack2">
            <img
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <div className="relative">
            ...
            {teamDropdownOpen && (
              <div className="absolute top-full left-0 right-0 mt-1 bg-BrandBlack2 border border-BrandGray2 rounded-md shadow-lg z-10 max-h-40 overflow-y-auto">
                {teams.map((team) => (
                  <button
                    key={team.id}
                    type="button"
        ...
        <div className="flex flex-col gap-1.5">
          <div className="flex items-center justify-between">
            ...
            {!showNewFolder && (
              <button
                ...
                className="text-BrandGray hover:text-BrandOrange text-[10px] font-DmSans transition-colors flex items-center gap-1"
              >
                <FaPlus className="text-[7px]" /> New Folder
              </button>
            )}
          </div>
          ...
          <div className="flex flex-col gap-0.5 max-h-36 overflow-y-auto hide-scroll">
            {folders.map((folder) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 px-2.5 py-2 rounded-lg text-left transition-colors ${
                    isSelected
                      ? "bg-BrandOrange/15 border border-BrandOrange/40"
                      : "bg-BrandGray2/15 border border-transparent hover:bg-BrandGray2/30"
                  }`}
                >
                  {isSelected ? (
                    <FaFolderOpen className="text-BrandOrange text-sm shrink-0" />
                  ) : (
                    <FaFolder className="text-BrandGray text-sm shrink-0" />
                  )}
                  <span
                    className={`text-xs font-DmSans flex-1 truncate ${
                      isSelected ? "text-BrandOrange font-medium" : "text-BrandWhite"
                    }`}
                  >
                    {folder.name}
                  </span>
                  <span className="text-[10px] text-BrandGray font-DmSans shrink-0">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <textarea
            ...
            className="w-full h-16 bg-BrandBlack2 border border-BrandGray rounded-md px-3 py-2 text-BrandWhite text-xs font-DmSans focus:outline-none focus:border-BrandOrange transition-colors resize-none"
            maxLength={200}
          />
        </div>
