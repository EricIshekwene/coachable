...
3. **Deploy command:** Run `powershell -ExecutionPolicy Bypass -File scripts/deploy-railway.ps1`. Railway CLI 5.x can omit the `server/` prefix from a project-root upload, causing `snapshot-target-unpack/server does not exist`. The script creates a clean archive from committed `server/` files and preserves the prefix. It also avoids `--ci`, which causes TLS connection errors on Windows.
