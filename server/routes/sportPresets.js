 ...
 * Public endpoint — no auth required. Returns only published (non-hidden) presets.
...
router.get("/:sport", async (req, res, next) => {
  try {
    const { rows } = await pool.query(
      "SELECT id, name, play_data FROM sport_presets WHERE LOWER(sport) = LOWER($1) AND is_hidden = false ORDER BY sort_order ASC, created_at ASC",
      [req.params.sport]
    );
